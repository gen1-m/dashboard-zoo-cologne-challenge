...
export const calculateAgeInYears = (birthdate: Date): number => {
  ...
  const result = Math.round(differenceInMilliseconds / (1000 * 60 * 60 * 24 * 365))
  if (result < 0)
  // Throw an error in case the birthdate is in the future.
    throw new Error('The birthdate provided can not be in the future.')

  if (result === 0) // handle an edge case where the difference may be 0
    return 1
...
export const calculateFoodForNextMonth = (
  ...
  return Math.round(calculateFoodPerDay(weight, height, fruit, gender, birthdate, species) * daysInMonth)
}
